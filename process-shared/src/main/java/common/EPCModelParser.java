...
//                 doc.getDocumentElement().getNodeName());
    ...
    public static void writeModel(String outputfile, Graph g) {
    	try {
    		PrintWriter output = new PrintWriter(new OutputStreamWriter(new FileOutputStream(outputfile), "UTF-8"));
...
//    				System.out.println(v.getID()+" - "+v.getLabel());
    				output.println("\t\t<function id=\""+v.getID()+"\">");
    				...
    				if (annotationMap != null && annotationMap.size() > 0) {
    					output.print("\t\t\t<attribute typeRef=\"configurationAnnotation\" value=\"");
    				...
    				output.println("\t\t<event id=\""+v.getID()+"\">");
    				output.println("\t\t\t<name>"+wrapLabels(v.getLabel())+"</name>");
    				HashMap<String, String> annotationMap = v.getAnnotationMap();
    				if (annotationMap != null && annotationMap.size() > 0) {
    					output.print("\t\t\t<attribute typeRef=\"configurationAnnotation\" value=\"");
    		...
    		for (Edge e : g.getEdges()) {
				...
				if (e.isLabelAddedToModel()) {
					...
					output.println("\t\t\t<attribute typeRef=\"configurationAnnotation\" value=\""+edgeLabel+"\" />");
				}
